...
void VC_100Hz(void)
{
    ...
    // Apply regen torque
    float regen_torque = RegenCalculator_calculate_regen();
    commanded_torque -= regen_torque;
    ...
    // Pack commanded torque value into pedal inputs message
    can_bus.vc_pedal_inputs.vc_pedal_inputs_torque_requested = main_bus_vc_pedal_inputs_vc_pedal_inputs_torque_requested_encode(limited_torque);
